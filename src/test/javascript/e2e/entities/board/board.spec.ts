...
describe('Board e2e test', () => {
  ...
  it('should load Boards', async () => {
    ...
    expect(await boardComponentsPage.getTitle()).to.eq('jhipsterSampleApp.board.home.title');
  ...
  it('should load create Board page', async () => {
    ...
    expect(await boardUpdatePage.getPageTitle()).to.eq('jhipsterSampleApp.board.home.createOrEditLabel');
  ...
  it('should create and save Boards', async () => {
    ...
    await promise.all([boardUpdatePage.setTitleInput('title'), boardUpdatePage.setNoticeListInput('noticeList')]);

    expect(await boardUpdatePage.getTitleInput()).to.eq('title', 'Expected Title value to be equals to title');
    expect(await boardUpdatePage.getNoticeListInput()).to.eq('noticeList', 'Expected NoticeList value to be equals to noticeList');
  ...
  it('should delete last Board', async () => {
    ...
    expect(await boardDeleteDialog.getDialogTitle()).to.eq('jhipsterSampleApp.board.delete.question');
